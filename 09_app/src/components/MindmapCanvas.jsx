...
export const MindmapCanvas = ({
  ...
  const draw = useCallback(() => {
    ...
    nodeSel.append("text")
      ...
      .attr("dy", (n) => (n.type === "term" ? nodeRadius(n.type) + 12 : nodeRadius(n.type) + 14))
      .attr("font-size", (n) => {
        if (n.type === "root") return 13;
        if (n.type === "scene") return 12;
        return 11;
      })
      .attr("font-weight", (n) => n.type === "term" ? 400 : 700)
      .attr("fill", (n) => nodeColor(n.type, false))
      ...
      {tooltip && (
        <div style={{ position: "fixed", left: tooltip.x + 12, top: tooltip.y - 10, background: "rgba(13,17,23,0.95)", border: "1px solid var(--border-color)", borderRadius: 10, padding: "12px 16px", zIndex: 200, maxWidth: 280, boxShadow: "0 8px 24px rgba(0,0,0,0.5)", pointerEvents: "none" }}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            ...
            <span style={{ padding: "2px 8px", borderRadius: 6, fontSize: 11, fontWeight: 600, background: "rgba(139,148,158,0.1)", color: "var(--text-secondary)", border: "1px solid var(--border-color)" }}>{tooltip.level === "Unrated" ? "레벨 미산출" : tooltip.level}</span>
          </div>
        </div>
      )}
    </div>
